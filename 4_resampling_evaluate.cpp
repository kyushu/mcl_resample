...
int main()
{
    ...
    for (int t = 0; t < steps; t++) {
        ...
        std::vector<double> robotx{myrobot.x};
        std::vector<double> roboty{myrobot.y};

        std::vector<double> lmkx, lmky, p2x, p2y, p3x, p3y;
        for(size_t i = 0; i<sizeof(landmarks) / sizeof(landmarks[0]); ++i)
        {
            lmkx.push_back(landmarks[i][0]);
            lmky.push_back(landmarks[i][1]);
        }
        for(size_t i=0; i < n; ++i)
        {
            p2x.push_back(p2[i].x);
            p2y.push_back(p2[i].y);

            p3x.push_back(p3[i].x);
